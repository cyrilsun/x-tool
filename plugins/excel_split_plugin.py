...
class ExcelSplitter:
    ...
    def read_excel_file(self, sheet_name: Optional[str] = None, header=0) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"✓ 成功读取: {os.path.basename(self.source_file)} - {sheet_name} ({len(df)} 行)")
            return {
            ...
            print(f"✗ 读取文件失败: {os.path.basename(self.source_file)}, 错误: {str(e)}")
            return {
                "sheet_name": sheet_name,
                "dataframe": None,
                "headers": [],
    ...
    def split_by_rows(self, df: pd.DataFrame, rows_per_file: int, preserve_header: bool = True) -> List[str]:
        ...
        print(f"开始按行数拆分，总共 {total_rows} 行，每个文件 {rows_per_file} 行，将生成 {num_splits} 个文件...")
        ...
        for i in range(num_splits):
            ...
            print(f"✓ 生成文件: {output_filename} ({len(split_df)} 行)")
    ...
    def split_by_columns(self, df: pd.DataFrame, columns_per_file: int, preserve_header: bool = True) -> List[str]:
        ...
        print(f"开始按列数拆分，总共 {total_columns} 列，每个文件 {columns_per_file} 列，将生成 {num_splits} 个文件...")
        ...
        for i in range(num_splits):
            ...
            print(f"✓ 生成文件: {output_filename} ({len(split_df.columns)} 列)")
    ...
    def split_by_field_value(self, df: pd.DataFrame, field_name: str, preserve_header: bool = True) -> List[str]:
        ...
        print(f"开始按字段值拆分，字段 '{field_name}' 有 {len(unique_values)} 个唯一值，将生成 {len(unique_values)} 个文件...")
