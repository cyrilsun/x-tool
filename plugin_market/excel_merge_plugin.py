...
class ExcelMerger:
    ...
    def _ensure_output_directory(self):
        ...
        if not os.path.exists(self.output_dir):
            ...
            logger.info(f"创建输出文件夹: {self.output_dir}")
    ...
    def get_excel_files(self) -> List[str]:
        ...
        if self.selected_files:
            logger.info(f"使用用户选择的 {len(self.selected_files)} 个Excel文件")
            return self.selected_files
        
        # 否则从源文件夹下查找所有Excel文件
        if not os.path.exists(self.source_dir):
            logger.info(f"警告: 源文件夹 {self.source_dir} 不存在")
            return []
        ...
        logger.info(f"在源文件夹中找到 {len(excel_files)} 个Excel文件")
        return excel_files
        ...
        try:
            ...
            for sheet_name in sheet_names:
                try:
                    ...
                    logger.info(f"✓ 成功读取: {file_name} - {sheet_name} ({len(df)} 行)")
                    result.append({
                    ...
                    logger.info(f"✗ 读取sheet失败: {file_name} - {sheet_name}, 错误: {str(e)}")
    ...
    def merge_excel_files(self, output_filename: Optional[str] = None, header=0, merge_header_rows=False, header_rows=1, header_mode="auto") -> dict:
        ...
        for i, file_path in enumerate(excel_files, 1):
            file_name = os.path.basename(file_path)
            logger.info(f"[{i}/{len(excel_files)}] 处理: {file_name}")
        ...
        if not all_dataframes:
            logger.info("没有成功读取任何Excel文件或sheet，无法进行合并")
            return {
                "success": False,
    ...
    def merge_with_sheet_separation(self, output_filename: Optional[str] = None, header=0, merge_header_rows=False, header_rows=1) -> dict:
        ...
        try:
            ...
            logger.info(f"总处理sheet数: {total_sheets}")
    ...
    def merge_single_file_sheets(self, header=0, merge_header_rows=False, header_rows=1, header_mode="auto") -> dict:
        ...
        for i, file_path in enumerate(excel_files, 1):
            ...
            try:
                ...
                sheets_data = self.read_excel_file(file_path, header=header, merge_header_rows=merge_header_rows, header_rows=header_rows)
                if not sheets_data:
                    logger.info(f"✗ 无法读取文件 {file_name} 中的任何sheet")
                    failed_files += 1
                    continue
                ...
                if not all_dataframes:
                    logger.info(f"✗ 文件 {file_name} 中没有可合并的数据")
                    failed_files += 1
                    continue
                ...
                logger.info(f"✓ 成功合并 {len(sheets_data)} 个sheet到文件: {output_path}")
                ...
                logger.info(f"✗ 合并文件 {file_name} 失败: {str(e)}")
                failed_files += 1
